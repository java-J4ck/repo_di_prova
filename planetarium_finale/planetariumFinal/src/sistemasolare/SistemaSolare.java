...
public class SistemaSolare {
	...
	public boolean trovaCorpoCeleste(CorpoCeleste c) {
		if(stella.equals(c)) return true;
	...
	public StringBuffer calcoloRotta(CorpoCeleste corpo1, CorpoCeleste corpo2) {
		...
		// trova di che tipo è corpo1
		if(corpo1.getId().charAt(1) == 'L') { // controlla se corpo1 è una luna
				luna=getLunaById(corpo1.getId()); // se corpo1 è una luna allora ci si sposta sul pianeta attorno al quale orbita
		...
		else if(corpo1.getId().charAt(1) == 'P'){ // controlla se corpo1 è un pianeta
			pianeta = getPianetaById(corpo1.getId()); // se è un pianeta allora si parte dal pianeta
			rotta.append(String.format("Pianeta (nome: %s, Id: %s)", pianeta.getName(), pianeta.getId()));
								
		}
		else {//il corpo 1 è una stella e ha le sue due classi
			rotta.append(String.format("Stella (nome: %s, Id: %s)", stella.getName(), stella.getId()));
			if(stella.getOrbitingPlanets().contains(corpo2)) { // controlla se corpo2 è un pianeta
			...
			else { // se corpo2 è una luna allora si va prima sul pianeta e infine sulla luna
		...
		if(pianeta.getOrbitingMoons().contains(corpo2) && corpo1.getId().charAt(0) != 'S') {// Controlla se il corpo2 sta sulle lune del pianeta, e lo fa solo se il corpo1 non è una stella 
		...
		else if(stella.getOrbitingPlanets().contains(corpo2)) { // controlla se corpo2 è un pianeta
			rotta.append(String.format("> Stella (nome: %s, Id: %s)", stella.getName(), stella.getId())); // se è un pianeta allora si va sulla stella e poi si raggiunge il pianeta
			distanzaTot += OperazioniCoordinate.distanzaPunti(pianeta.getAbsolute_coords(), stella.getAbsolute_coords());
			rotta.append(String.format("> Pianeta (nome: %s, Id: %s)", corpo2.getName(), corpo2.getId()));
			distanzaTot += OperazioniCoordinate.distanzaPunti(((Pianeta)corpo2).getAbsolute_coords(), stella.getAbsolute_coords());
		}
		else if(stella.equals(corpo2)) { // se corpo2 è una stella, allora si va sulla stella
			rotta.append(String.format("> Stella (nome: %s, Id: %s)", stella.getName(), stella.getId()));
			distanzaTot+=OperazioniCoordinate.distanzaPunti(pianeta.getAbsolute_coords(), stella.getAbsolute_coords());
		}
		else { // se corpo2 è una luna allora si va prima sulla stella, poi sul pianeta e infine sulla luna
			for(Pianeta pianeta2: stella.getOrbitingPlanets()) {
				for(Luna lun: pianeta2.getOrbitingMoons()) { // for annidato per cercare tra tutte le lune 
					if(lun.equals(corpo2)) { // se la luna viene trovata restituisce il percorso
						rotta.append(String.format("> Stella (nome: %s, Id: %s)",stella.getName(),stella.getId()));
